...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save file to public/Hero directory
    ...
    try {
      ...
      if (imageId && originalFilename) {
        ...
        await connection.execute(
          'UPDATE hero_images SET name = ?, alt_text = ?, description = ?, updated_at = NOW() WHERE id = ?',
          [name, alt, description || null, imageId]
        );
        
        isUpdate = true;
        console.log(`Updated existing hero image ID ${imageId}: ${filename}`);
      } else {
        // New image - check if filename already exists
        const [existingResult] = await connection.execute(
          'SELECT id, display_order FROM hero_images WHERE filename = ?',
          [filename]
        );
        ...
        if (Array.isArray(existingResult) && existingResult.length > 0) {
          ...
          await connection.execute(
            'UPDATE hero_images SET name = ?, alt_text = ?, description = ?, updated_at = NOW() WHERE filename = ?',
            [name, alt, description || null, filename]
          );
          
          isUpdate = true;
          console.log(`Updated existing hero image: ${filename}`);
        } else {
          // Create new record
          let displayOrder = 1;
          if (order && !isNaN(parseInt(order))) {
            ...
            const [orderResult] = await connection.execute(
              'SELECT COALESCE(MAX(display_order), 0) + 1 as next_order FROM hero_images'
            );
            displayOrder = (orderResult as { next_order: number }[])[0]?.next_order || 1;
          }

          const [result] = await connection.execute(
            'INSERT INTO hero_images (name, filename, alt_text, description, display_order, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())',
            [name, filename, alt, description || null, displayOrder, true]
          );

          const insertResult = result as { insertId: number };
          imageId = insertResult.insertId;
          console.log(`Created new hero image: ${filename}`);
        }
      }
      ...
      return NextResponse.json({
        message: isUpdate ? 'Hero image updated successfully' : 'Hero image uploaded successfully',
        imageId: imageId,
        filename: filename,
        url: `/Hero/${filename}`,
        isUpdate: isUpdate
