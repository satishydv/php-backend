...
class Admin_gallery_upload extends CI_Controller {
    ...
    /**
     * POST /api/admin/gallery/upload-image - Upload gallery image
     */
    public function upload_image() {
        ...
        try {
            ...
            $upload_dir = FCPATH . 'public/Gallery/';
            ...
            if ($image_id && $original_filename) {
                ...
                if ($existing) {
                    ...
                    $success = $this->Gallery_image_model->update_by_filename($filename, $image_data);
                    $is_update = true;
                    $db_image_id = $existing['id'];
                } else {
                    // Create new record
                    $db_image_id = $this->Gallery_image_model->create($image_data);
            ...
            if ($success) {
                $response = [
                    'message' => $is_update ? 'Image updated successfully' : 'Image uploaded successfully',
                    'imageId' => $db_image_id,
                    'filename' => $filename,
                    'url' => $this->config->item('base_url') . "public/Gallery/{$filename}",
                    'isUpdate' => $is_update
                ];
            ...
            $this->output->set_status_header(500)->set_output(json_encode(['error' => 'Failed to upload image']));
        }
    }
